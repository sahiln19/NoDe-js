...
let getObject = function (request, response) {
    ...
    database.collection('collection').find(condition, fields).limit(count).toArray(function (error, result) {
...
let updateObject = function (request, response) {
    if (request.body.city === undefined || request.body.newcity === undefined || request.body.newpincode === undefined) {
        ...
        dbPromise.then((database) => {
            ...
            database.collection('collection').updateMany(condition, updatedDocument, function (error, result) {
...
let deleteObject = function (request, response) {
    dbPromise.then((database) => {
        ...
        database.collection('collection').deleteMany(condition, function (error, result) {
            if (error)
